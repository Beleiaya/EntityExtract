...
class NerProcessor(DataProcessor):
    ...
    def get_predicate_labels(self):
        """See base class."""
        return ['Instrument-Agency', 'Product-Producer', 'Content-Container', 'Entity-Destination', 'Cause-Effect', 'Member-Collection', 'Component-Whole', 'Other', 'Entity-Origin', 'Message-Topic']
...
def convert_single_example(ex_index, example, label_list, predicate_label_list,max_seq_length,
                           tokenizer):
    ...
    if ex_index < 5:
        ...
        tf.logging.info("predicate_label: %s (id = %d)" % (example.text_b, predicate_label_id))
    ...
    feature = InputFeatures(
        ...
        label_id=label_ids,
        predicate_label_id=predicate_label_id,
        is_real_example=True)
    return feature, ntokens, label_ids


def file_based_convert_examples_to_features(
        examples, label_list, predicate_label_list,max_seq_length, tokenizer, output_file):
    ...
    for (ex_index, example) in enumerate(examples):
        ...
        a=0
        c=a+2
        feature , ntokens, label_ids= convert_single_example(ex_index, example, label_list,predicate_label_list,max_seq_length, tokenizer)
        ...
        features["predicate_label_ids"] = create_int_feature([feature.predicate_label_id])
...
def file_based_input_fn_builder(input_file, seq_length, is_training,
    ...
    name_to_features = {
        ...
        "predicate_label_ids": tf.FixedLenFeature([], tf.int64),
...
def create_model(bert_config, is_training, input_ids, input_mask, segment_ids,
                 labels,predicate_label, num_labels, num_predicate_labels,use_one_hot_embeddings,crf):
    ...
    with tf.variable_scope("loss"):
        ...
        output_weights = tf.get_variable(
            "output_weights", [num_predicate_labels, hidden_size],
            initializer=tf.truncated_normal_initializer(stddev=0.02))

        output_bias = tf.get_variable(
            "output_bias", [num_predicate_labels], initializer=tf.zeros_initializer())
        if is_training:
            # I.e., 0.1 dropout
            output_layer = tf.nn.dropout(output_layer, keep_prob=0.9)

        logits_predicate = tf.matmul(output_layer, output_weights, transpose_b=True)
        logits_predicate = tf.nn.bias_add(logits_predicate, output_bias)
        probabilities_predicate  = tf.nn.softmax(logits_predicate, axis=-1)
        log_probs = tf.nn.log_softmax(logits_predicate, axis=-1)

        one_hot_labels = tf.one_hot(predicate_label, depth=num_predicate_labels, dtype=tf.float32)
        per_example_loss = -tf.reduce_sum(one_hot_labels * log_probs, axis=-1)
        loss_predicate = tf.reduce_mean(per_example_loss)
        predict_predicate = tf.math.argmax(probabilities_predicate, axis=-1)
        '-----2.添加全连接层，计算损失函数--------'
        ...
        predict=tf.reshape(predict, [-1, output_layer.shape[-2].value])


        return (loss_predicate+loss, logits,logits_predicate,predict,predict_predicate)


def model_fn_builder(bert_config, num_labels,num_predicate_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        (total_loss, logits,logits_predicate,predicts,predicts_predicate) = create_model(
            bert_config, is_training, input_ids, input_mask, segment_ids, label_ids,
            predicate_label_ids,num_labels, num_predicate_labels,use_one_hot_embeddings,crf=crf)
        ...
        if mode == tf.estimator.ModeKeys.TRAIN:
            ...
            def metric_fn(label_ids, predicate_label_ids,logits,segment_ids,logits_predicate,is_real_example ):
                predicate_predicate = tf.argmax(logits_predicate, axis=-1, output_type=tf.int32)
                accuracy_predicate = tf.metrics.accuracy(
                    labels=predicate_label_ids, predictions=predicate_predicate, weights=is_real_example)
                ...
                eval_metrics = {
                    ...
                    'f1': f1,
                    'f2': f2,
                    'accuracy_predicate':accuracy_predicate

                }
                return eval_metrics
            eval_metrics = (metric_fn, [label_ids,predicate_label_ids, logits,segment_ids,logits_predicate,is_real_example])
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(
                mode=mode,
                loss=total_loss,
            ...
            predict_dict = {'predictions': predicts,
                            'prediction_predicate':predicts_predicate}
            output_spec = tf.contrib.tpu.TPUEstimatorSpec(
                mode=mode,
...
def main(_):
    ...
    OUTPUT_DIR = 'gs://storage_hx/test/models/joint_relation/data2'
    CRF=False
    DO_TRAIN=False
    DO_EVAL=False
    DO_PREDICT=True
    ...
    if DO_TRAIN and DO_EVAL:
        ...
        file_based_convert_examples_to_features(
            train_examples, label_list,predicate_label_list, MAX_SEQ_LENGTH, tokenizer, train_file)
        ...
        file_based_convert_examples_to_features(
            eval_examples, label_list,predicate_label_list, MAX_SEQ_LENGTH, tokenizer, eval_file)
    ...
    '-----------------7.预测---------------'
    if DO_PREDICT:
        predict_examples = processor.get_dev_examples(TASK_DATA_DIR + 'eval/')
        ...
        batch_tokens, batch_labels=file_based_convert_examples_to_features(predict_examples, label_list,
                                                predicate_label_list,MAX_SEQ_LENGTH, tokenizer,
                                                predict_file)
        ...
        temp= estimator.predict(input_fn=predict_input_fn)
        result = list(temp)
        result2=[pred['prediction_predicate'] for pred in result]
        result = [pred['predictions'] for pred in result]
        deal_result=[]
        for item in result:
            deal_result.extend(item)
        ...
        Writer(output_predict_file, deal_result, batch_tokens, batch_labels, id2label,crf=CRF)

        result = [pred['prediction_predicate'] for pred in temp]
        output_predict_file = os.path.join(OUTPUT_DIR, "test_results_predicate.tsv")
        with tf.gfile.GFile(output_predict_file, "w") as wf:
            for (i, prediction) in enumerate(result2):
                wf.write(str(predicate_label_id2label[prediction])+'\n')
        with tf.gfile.GFile("test_results_predicate.tsv", "w") as wf:
            for (i, prediction) in enumerate(result2):
                wf.write(str(predicate_label_id2label[prediction])+'\n')
        print('ok')
