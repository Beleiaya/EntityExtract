...
class PredicateProcessor(DataProcessor):
    ...
    def get_train_examples(self, data_dir):
        ...
        return self._create_examples(text_list,predicate_list, "train")
    ...
    def get_dev_examples(self, data_dir):
        ...
        with tf.gfile.GFile(os.path.join(data_dir, "predicate.txt"))as f:
            predicate_list = f.readlines()
        return self._create_examples(text_list, predicate_list, "dev")
    def get_test_examples(self, data_dir):
        """See base class."""
        with tf.gfile.GFile(os.path.join(data_dir, "text.txt")) as f:
            text_list = f.readlines()
        with tf.gfile.GFile(os.path.join(data_dir, "predicate.txt"))as f:
            predicate_list = f.readlines()
        return self._create_examples(text_list, predicate_list, "test")
    ...
    def _create_examples(self, text_list,predicate_list, set_type):
        """
        :type text_list:List[str]
        :type predicate_list[str]
        Creates examples for the training and dev sets.
        """
        examples = []
        for i,(text,predicate) in enumerate(zip(text_list,predicate_list)):

            guid = "%s-%s" % (set_type, i)
            text_a = tokenization.convert_to_unicode(text.strip())
            text_b = None
            if set_type == "test":
                label = "other"
            else:
                label = tokenization.convert_to_unicode(predicate.strip())
            examples.append(
...
def model_fn_builder(bert_config, num_labels, init_checkpoint, learning_rate,
    ...
    def model_fn(features, labels, mode, params):  # pylint: disable=unused-argument
        ...
        predicts = tf.math.argmax(probabilities, axis=-1)
...
def main(_):
    ...
    OUTPUT_DIR = 'gs://storage_hx/test/models/classfier_predicate_data2'
    ...
    processor = processors['predicate']()
    ...
    if DO_TRAIN and DO_EVAL:
        ...
        while current_step < num_train_steps:
            ...
            estimator.train(input_fn=train_input_fn, max_steps=next_checkppint)
    ...
    if DO_PREDICT:
        ...
        file_based_convert_examples_to_features(
            predict_examples, label_list, MAX_SEQ_LENGTH, tokenizer, predict_file)
        ...
        for (i, label) in enumerate(label_list):
            id2label[i] = label
        output_predict_file = os.path.join('/content/data4relation_2/predict', "predicate.txt")
        with tf.gfile.GFile(output_predict_file, "w") as wf:
            for (i, prediction) in enumerate(result):
                wf.write(str(id2label[prediction]) + '\n')
        print('ok')
